...
type Section = 'menu' | 'goals' | 'employees' | 'roles'
...
const DEFAULT_PERMISSIONS: Record<string, { label: string; perms: Record<string, boolean> }> = {
  ...
  technician: { label: 'Technician', perms: { 'View dashboards': true, 'Edit patients': false, 'Unlocked notes': false, 'Edit employees & roles': false, 'Manage goals': false } },
    ...
    const MENU = [
      {
        ...
        icon: Target,
        title: 'Company goals',
        desc: 'Create and remove the targets that drive the Executive overview alerts.',
        stat: `${goals.length} goals`,
        warn: offTarget > 0 ? `${offTarget} off target` : undefined,
      },
      {
        id: 'employees' as const,
        icon: Users,
        title: 'Employees',
        desc: 'Add, edit, or remove team members and their roles.',
      ...
      },
      {
        ...
        desc: 'Control what each role can see and do across the dashboard.',
        stat: `${Object.keys(permissions).length} roles · ${PERM_COLUMNS.length} permissions`,
      },
    ]
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          {MENU.map(({ id, icon: Icon, title, desc, stat, warn }) => (
            <button
              ...
              className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-brand-400 hover:shadow-md"
            >
              ...
              <div className="mt-4 flex flex-wrap gap-1.5">
                ...
                {warn && (
                  <span className="rounded-full bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-600">{warn}</span>
                )}
              </div>
            </button>
          ))}
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-slate-400">
          ...
          <span className="font-semibold text-ink-900">{SECTION_TITLES[section]}</span>
        </span>
      </div>
        ...
        <Card
          ...
          action={
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-brand-500 px-3 py-1.5 text-xs font-semibold text-ink-900 transition hover:bg-brand-400"
            >
          ...
          {showGoalForm && (
            <div className="mb-4 flex flex-wrap items-end gap-2 rounded-xl border border-brand-200 bg-brand-50/50 p-3">
              <div>
                ...
                <input value={draft.label} onChange={(e) => setDraft({ ...draft, label: e.target.value })} placeholder="e.g. EBOOs per month" className={`w-44 ${inputCls}`} />
              </div>
              <div>
                <p className="mb-1 text-[11px] font-medium text-slate-500">Area</p>
                <input value={draft.area} onChange={(e) => setDraft({ ...draft, area: e.target.value })} placeholder="e.g. Nurses" className={`w-28 ${inputCls}`} />
              </div>
              <div>
                <p className="mb-1 text-[11px] font-medium text-slate-500">Current</p>
                <input value={draft.value} onChange={(e) => setDraft({ ...draft, value: e.target.value })} placeholder="0" inputMode="numeric" className={`w-20 ${inputCls}`} />
              </div>
              <div>
                <p className="mb-1 text-[11px] font-medium text-slate-500">Target</p>
                <input value={draft.target} onChange={(e) => setDraft({ ...draft, target: e.target.value })} placeholder="100" inputMode="numeric" className={`w-20 ${inputCls}`} />
              </div>
              <label className="flex items-center gap-1.5 pb-1.5 text-xs text-slate-500">
                <input type="checkbox" checked={draft.lowerIsBetter} onChange={(e) => setDraft({ ...draft, lowerIsBetter: e.target.checked })} />
                Lower is better
              </label>
              <button onClick={addGoal} className="rounded-lg bg-brand-500 px-3 py-1.5 text-sm font-semibold text-ink-900 transition hover:bg-brand-400">
                Save goal
              </button>
            </div>
          )}
          ...
          {showEmpForm && (
            <div className="mb-4 flex flex-wrap items-end gap-2 rounded-xl border border-brand-200 bg-brand-50/50 p-3">
              <div>
                ...
                <input value={empDraft.name} onChange={(e) => setEmpDraft({ ...empDraft, name: e.target.value })} placeholder="e.g. Laura" className={`w-44 ${inputCls}`} />
              </div>
              <div>
                <p className="mb-1 text-[11px] font-medium text-slate-500">Role</p>
                <select value={empDraft.role} onChange={(e) => setEmpDraft({ ...empDraft, role: e.target.value })} className={inputCls}>
                  {ROLE_OPTIONS.map((r) => (
              ...
              <div>
                ...
                <input value={empDraft.utilization} onChange={(e) => setEmpDraft({ ...empDraft, utilization: e.target.value })} placeholder="0" inputMode="numeric" className={`w-20 ${inputCls}`} />
              </div>
              <button onClick={addEmployee} className="rounded-lg bg-brand-500 px-3 py-1.5 text-sm font-semibold text-ink-900 transition hover:bg-brand-400">
                Save employee
              </button>
            </div>
          )}
                ...
                {employees.map((e) => {
                  ...
                  return (
                    <tr key={e.id} className="border-b border-slate-100 last:border-0">
                      <td className="py-2">
                        {isEditing ? (
                          <input value={edit.name} onChange={(ev) => setEdit({ ...edit, name: ev.target.value })} className={`w-44 ${inputCls}`} />
                        ) : (
                      ...
                      <td className="py-2">
                        {isEditing ? (
                          <select value={edit.role} onChange={(ev) => setEdit({ ...edit, role: ev.target.value })} className={inputCls}>
                            {ROLE_OPTIONS.map((r) => (
                              <option key={r} value={r}>{r}</option>
                            ))}
                          </select>
                      ...
                      <td className="py-2 text-right tabular-nums text-slate-600">
                        {isEditing ? (
                          <input value={edit.utilization} onChange={(ev) => setEdit({ ...edit, utilization: ev.target.value })} inputMode="numeric" className={`w-16 text-right ${inputCls}`} />
                        ) : (
                          `${e.utilizationPct}%`
                        )}
                      </td>
